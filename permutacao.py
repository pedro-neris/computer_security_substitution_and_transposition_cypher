...
def decripta_colunar(cifrado: str, chave: str):
    ...
    if len(cifrado) < len(chave):
        raise ValueError("A chave deve ser menor ou igual que a mensagem")
    num_linhas = len(cifrado) // len(chave)
    resto_linhas = len(cifrado) % len(chave)
    tupla_linhas = []
    for letra in chave:
        tupla_linhas.append((letra, num_linhas))
    for i in range(resto_linhas):
        tupla_linhas[i] = (tupla_linhas[i][0], tupla_linhas[i][1] + 1)
    dicionario_chave = {letra: "" for letra in chave}
    tupla_linhas.sort(key=lambda x: x[0])
    index = 0
    for tupla in tupla_linhas:
        letra = tupla[0]
        dicionario_chave[letra] = cifrado[index : tupla[1] + index]
        index += tupla[1]
    mensagem_decriptada = ""
    qnt_iteracoes = math.ceil(len(cifrado) / len(chave))
    for j in range(qnt_iteracoes):
        for i in range(len(chave)):
            letra = chave[i]
            if j < len(dicionario_chave[letra]):
                letra_atual = dicionario_chave[letra][j]
...
cifra = encripta_colunar("analise de frequencia", "triz")
print(cifra)
#print(decripta_colunar("psrmietolaabtognciomofrdmiarddxcnoaleuocriuapiea", "confirma"))
forca_bruta_colunar(cifra)